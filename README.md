...
Desde PRO-59, las funciones de Convex en sí (`clients.*`, `notes.*`, `followUps.*`, `sales.*`) también exigen autenticación — ya no basta con la URL del deployment y el nombre de la función. La cookie de sesión es httpOnly a propósito (para que un XSS no pueda robarla), lo que significa que el JavaScript del navegador nunca puede leerla directamente; en su lugar, cada una de esas 14 funciones exige un **accessToken** de corta duración, derivado de la sesión ya validada, que sí puede viajar en las llamadas de `convex/react`. El riesgo que este README aceptaba antes mientras el deployment tuviera datos ficticios ya no aplica a esta superficie.
