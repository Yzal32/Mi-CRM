...
### Cómo funciona la recuperación de contraseña (PRO-67)

Tercer flujo de contraseña, distinto de `users.changePassword` (usuario ya logueado, PRO-57) y `users.resetEmployeePassword` (lo hace la Dueña sobre un empleado, PRO-45): aquí el usuario no tiene sesión y demuestra su identidad poseyendo el email registrado — el enlace del correo hace de credencial.

- `app/recuperar-contrasena` (pide el enlace) → `app/restablecer-contrasena/[token]` (establece la contraseña nueva). Ambas rutas están excluidas de la redirección a `/login` en `proxy.ts` — igual que `/login` y las de Google, un usuario **sin sesión** es precisamente quien necesita alcanzarlas.
- `passwordReset.requestPasswordReset` (Convex `action` pública): genera el token vía `internal.passwordReset.createResetForEmail` (`internalMutation`) y, si existe una cuenta activa con ese email, envía el correo con `internal.email.sendPasswordResetEmail`. Responde siempre igual (sin error) exista o no la cuenta — mismo criterio de no revelar cuentas que `sessions.login`.
- **`APP_URL` es también variable de entorno de Convex** (nueva, además de la ya existente en Railway/`.env.local` para las rutas de Google, PRO-65): `requestPasswordReset` construye el enlace del email a partir de `process.env.APP_URL` de **Convex**, nunca de un argumento de la action — es una action pública, alcanzable directamente por cualquier cliente Convex sin pasar por el Server Action de Next.js, así que aceptar la URL base como argumento habría permitido a un atacante hacer que el enlace de recuperación de una víctima apuntara a un dominio propio y robar el token (hallazgo de la ronda de auditoría de PRO-67). Si `APP_URL` no está configurada en Convex (o no es una URL http(s) válida), la action falla en cerrado antes de mirar el email, para que el fallo tampoco sirva de oráculo de qué cuentas existen. Se configura igual que el resto de variables de Convex: `npx convex env set APP_URL https://mi-crm-production-d80f.up.railway.app`.
- Tabla `passwordResets` (`convex/schema.ts`): mismo patrón de token opaco que `sessions`/`accessTokens` (`convex/model/tokens.ts`, extraído de `sessions.ts` en esta tarea) — token de 32 bytes aleatorios, solo se persiste su hash SHA-256, caduca a la hora (`PASSWORD_RESET_TOKEN_TTL_MS`). Un solo uso: `users.resetPasswordWithToken` rota las sesiones del usuario (igual que `changePassword`) y borra todos los tokens de recuperación pendientes de esa cuenta, no solo el usado.
...
- **Riesgos aceptados** (documentados explícitamente en la auditoría de PRO-67, sin mitigación en esta tarea): sin rate-limiting sobre `requestPasswordReset` (mismo criterio que `login`/`loginWithGoogle`); a diferencia de `login` (que usa `DUMMY_HASH` para tiempo constante), `requestPasswordReset` sí tiene una diferencia de tiempo observable entre "el email existe" (hace un `fetch` real a Resend) y "no existe" (retorno inmediato).
