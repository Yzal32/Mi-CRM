...
describe("notes.create / notes.unfeature (capa pública)", () => {
  ...
  test("create expone TEXT_REQUIRED en error.data.code", async () => {
    ...
    const error = await captureError(t.mutation(api.notes.create, { token, clientId, text: "   " }));
    expect(error.data.code).toBe("TEXT_REQUIRED");
  });

  test("create rechaza sin token válido con UNAUTHENTICATED, sin escritura parcial", async () => {
    const t = convexTest(schema, modules);
    const clientId = await t.run((ctx) => ctx.db.insert("clients", { name: "Cliente Test" }));

    const error = await captureError(t.mutation(api.notes.create, { token: "a".repeat(64), clientId, text: "Nota" }));
    expect(error.data.code).toBe("UNAUTHENTICATED");
    // Ninguna nota debe haberse creado — requireAccessToken corta antes de
    // llegar a createNote.
  ...
  test("unfeature delega en el modelo", async () => {
    ...
    const token = await issueTestAccessToken(t);
    const clientId = await t.run((ctx) => ctx.db.insert("clients", { name: "Cliente Test" }));
    const noteId = await t.mutation(api.notes.create, { token, clientId, text: "Nota", featured: true });

    await t.mutation(api.notes.unfeature, { token, noteId });
