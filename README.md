...
- **Un código de 6 dígitos (1.000.000 de combinaciones) es un secreto mucho más débil que el token de 256 bits del enlace de PRO-67**, así que hacen falta varios controles para que sea seguro (ver `convex/model/passwordReset.ts`):
  ...
  - `passwordReset.resetPassword` es una `action` fina que envuelve una única `internalMutation` (`verifyAndApplyReset`): verificación del código, cambio de contraseña y borrado del código viven en la **misma** transacción — separarlas en dos mutations (una que "autoriza" un `userId` y otra que lo consume después) se probó y se descartó: bajo concurrencia, dos peticiones con el mismo código correcto podían pasar ambas la verificación antes de que ninguna lo consumiera, rompiendo el "un solo uso". `verifyAndApplyReset` nunca lanza en el camino de "código incorrecto" (devuelve `"invalid"` con normalidad) para que el incremento de `attempts` se comite igual — Convex revierte **todos** los writes de una mutation que lanza, así que lanzar ahí perdería ese incremento. Solo la `action` decide, después del commit, si convertir `"invalid"` en el error `RESET_CODE_INVALID`.
